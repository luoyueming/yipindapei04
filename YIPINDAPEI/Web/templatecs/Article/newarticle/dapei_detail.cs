...
{
    public class dapei_detail : Base.BasePage
    {
        ...
        private void Get_Top10_Zixun(NVelocity.VelocityContext context)
        {
            ...
            string sql = "select top " + pagesize + " id,title,types,New_Zixun_intro_short,isimg,img_url,New_Zixun_Price,isnull((select product_name from HRENH_PRODUCT_PLATINFO where id=a.product_plat_id),'') as platname,isnull((select Produce_Img from HRENH_PRODUCT_PLATINFO where id=a.product_plat_id),'') as platimg,New_Zixun_Idlist from hrenh_article a where " + where + "order by istop desc,edittime desc";
        ...
        private void Get_Top5_Zixun(NVelocity.VelocityContext context)
        {
            ...
            //string sql = "select b.*,(select TypeRename from HRENH_ARTICLE_TYPE where state=1 and ID=b.parents_id) as prname,(select TypeName from HRENH_ARTICLE_TYPE where state=1 and ID=b.parents_id) as ptypename,(select min(id) from HRENH_ARTICLE_TYPE where state=1 and pid=b.parents_id) as first_type_id from ";
            //sql += "(";
            //sql += "select top " + pagesize + " id,TypeImg,TypeName,TypeRename,PID,edittime,(select PID from HRENH_ARTICLE_TYPE where state=1 and ID=a.PID) as parents_id from HRENH_ARTICLE_TYPE a where state=1 and ISTOP=1 and Type_parent=1 and id>370";
            //sql += ") b order by edittime desc";


            string sql = "select top " + pagesize + " a.id,a.typename,a.lvl,b.typeimg,b.TypeRename from return_article_all_son_typelist(370,1) a,hrenh_article_type b where a.lvl=2 and b.state=1 and b.istop=1 and a.id=b.id and charindex(','+convert(varchar,SEASON_ID,10)+',','," + season_require_id + ",')>0 order by edittime desc";
            ...
            if (ds != null && ds.Tables[0].Rows.Count > 0)
            {
                ...
                for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
                {
                    ...
                    string article_url = "href=\"/zhuanti/" + son_rname + ".html\" target=\"_blank\" title=\"" + article_title + "\"";
