    ...
    MScroll: function(settings) {
        ...
        function AutoScroll() {
            ...
            $(config.inner).stop().animate({ "margin-left": parseInt(marginLeft) - config.stepWidth }, config.animateTime, function() {
                var n = $(".nn").toArray(); //转换为对象数组
                var k = $.grep(n, function(i, j) { return j > 0; }); //筛选
                var temp = $.grep(n, function(i, j) { return j > 0; }, true); //筛选
                //$(config.inner).children(".Lunb").html(""); //清空
                $(k).each(function(i, j) {
                    $(config.inner).children(".Lunb").append(j); //重新拼装内容
                });
                $(config.inner).children(".Lunb").append(temp); //拼装被筛选出去的对象
