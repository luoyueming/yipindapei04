...
{
    public class CheckIsLogin_HongRenHui
    {
        ...
        public static bool IsLogin(string cookiename)
        {
            ...
            try
            {
                if (CookieManage_HongRenHui.IsHaveCookie(cookiename))
                {
                    RED_USERID = Convert.ToInt32(CookieManage_HongRenHui.GetCookieValue(cookiename));
                    if (user_bll_h.Exists(RED_USERID))
                    {
                        //结束时间
                        DateTime end_time = CookieManage_HongRenHui.GetCookieValue(cookiename,2);
        ...
        public static int GetUserID(string cookiename)
        {
            ...
            {
                if (!String.IsNullOrEmpty(CookieManage_HongRenHui.GetCookieValue(cookiename)))
                {
                    uid = Convert.ToInt32(CookieManage_HongRenHui.GetCookieValue(cookiename));
                }
            }
        ...
        /// <returns></returns>
        public static string Get_User_CenterUrl_HongRenHui()
        {
            int uid = GetUserID("RED_HREN_USERID");
