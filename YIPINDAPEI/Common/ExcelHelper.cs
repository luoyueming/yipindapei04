...
namespace NewXzc.Common
{
    public class ExcelHelper
    {
        ...
        {
            //如果文件夹不存在，则创建
            if (!Directory.Exists(StringPlus.GetExportPath()))
            {
                Directory.CreateDirectory(StringPlus.GetExportPath());
            }
            ...
            //string filename = System.Web.HttpContext.Current.Server.MapPath("DownLoad/aaa.zip");
                ...
                for (int i = (HeaderRowIndex + 1); i <= sheet.LastRowNum; i++)
                {
                    try
                    {
                        ...
                        for (int j = row.FirstCellNum; j <= cellCount; j++)
                        {
                            ...
                            {
                                //wl.WriteLogs(ex.ToString());.WriteLogs(exception.ToString());
                            }
                        }
